...
object OpenRepoUpdate : Update<OpenRepoModel, OpenRepoEvent, OpenRepoEffect> {
  override fun update(
    ...
    return when (event) {
      ...
      is GitRepositoryFound -> dispatch(setOf(UpdateRecentRepositories(event.path), OpenRepository(event.path)))
      is GitRepositoryNotFound -> dispatch(setOf(ShowNotAGitRepositoryError(event.path)))
